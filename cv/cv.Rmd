...
```{r}
skills %>% 
  detailed_entries(what = skill, with = category, .protect = TRUE)
```
\pagebreak
...
```{r}
training %>% 
  detailed_entries(what = what, with = with, when = when, where = where, why = why, .protect = TRUE)
```
# Teaching

```{r}
teaching %>% 
  detailed_entries(what = what, when = when, with = with, where = where, why = why, .protect = TRUE)
```
...
```{r}
software %>% 
  detailed_entries(what = what, with = package, when = when, where = where, why = why, .protect = TRUE)
```
...
dashboards %>% 
  dplyr::mutate(with = what, 
                what = why) %>% 
  detailed_entries(what = what, with = with, when = when, where = where, .protect = TRUE)
```
...
```{r}
service %>% 
  detailed_entries(with = type, why = details, .protect = TRUE)
```
